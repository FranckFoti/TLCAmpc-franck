...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Train the AV-LSTM trajectory prediction model.",
    ...
    p.add_argument(
        ...
        required=True,
        metavar="DIR",
        help="Directory containing NPZ training files (Phase 20 output).",
    )
    p.add_argument(
        "--output",
        type=Path,
        required=True,
        metavar="PATH",
        help="Destination path for the saved .pt model checkpoint.",
    )
    p.add_argument(
        "--epochs",
        type=int,
    ...
    p.add_argument(
        ...
        default=20,
        metavar="M",
        help="History length in steps (informational; determined by NPZ data shape).",
    )
    p.add_argument(
        "--T",
        type=int,
        default=80,
        metavar="T",
        help="Prediction horizon in steps (passed to AVLSTMModel).",
    )
    return p
...
def main() -> None:
    ...
    log.info(
        "Starting training — dataset_size=%d, epochs=%d, lr=%s, batch_size=%d",
        len(trainer.dataset),
        args.epochs,
        args.lr,
        args.batch_size,
    ...
    log.info(
        "Training complete — first_loss=%.4f, last_loss=%.4f, min_loss=%.4f",
        losses[0],
        losses[-1],
        min(losses),
    )
