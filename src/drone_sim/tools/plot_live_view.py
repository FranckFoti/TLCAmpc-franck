...
def cut_after_first_gap(df: pd.DataFrame, col: str = "horizon") -> pd.DataFrame:
...
def plot_subplot_for_n(ax: plt.Axes, df_N: pd.DataFrame) -> None:
   ...
   if infeasible_mask.any():
      ...
      h_inf_all = df_N.loc[infeasible_mask, "horizon"].astype(float).sort_values().to_numpy()

      tail_start: float | None = None
      seen_valid = False
      # df_N is already sorted by horizon
      for h, is_inf in zip(horizons, infeasible_mask.to_numpy()):
         if not is_inf:
            seen_valid = True
         elif is_inf and seen_valid:
            tail_start = float(h)
            break

      max_h = float(horizons.max())

      # If we have a first infeasible after at least one valid point, shade from there to the end.
      if tail_start is not None:
         ax.axvspan(tail_start - 0.5, max_h + 0.5, color="grey", alpha=0.3, zorder=0)
         h_inf_early = h_inf_all[h_inf_all < tail_start]
      else:
         h_inf_early = h_inf_all

      # For earlier infeasible horizons (before the tail), use block merging.
      if h_inf_early.size > 0:
         max_gap_horizons = 3  # max difference in H between consecutive infeasible points to still merge (=> up to 2 gaps)
         start = h_inf_early[0] - 0.5
         prev_h = h_inf_early[0]
         for h in h_inf_early[1:]:
            if h - prev_h <= max_gap_horizons:
               # Extend current grey block
               prev_h = h
            else:
   ...
   ax2 = ax.twinx()
   ax2.plot(df_N["horizon"], df_N["sim_time_s"], marker="o", color="tab:purple", linewidth=1, markersize=4, label="overall time [s]")
