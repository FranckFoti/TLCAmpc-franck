...
class Simulator:
    ...
    def from_config(cls, cfg: ScenarioConfig) -> "Simulator":
        ...
        physics = create_physics(
            {"type": cfg.physics.type, "params": {"dt": cfg.dt, **cfg.physics.params}}
        )
        ...
        for dcfg in cfg.drones:
            spec = dcfg.controller or cfg.controller
            controller = create_controller(
                {"type": spec.type, "params": {"dt": cfg.dt, **spec.params}}
            )
            start = np.asarray(dcfg.start, dtype=float)
            x0 = np.zeros(6, dtype=float)
            x0[:3] = start

            route = Route(
                waypoints=[np.asarray(w, dtype=float) for w in dcfg.waypoints],
                target=np.asarray(dcfg.target, dtype=float),
            )
            drone_color = _normalize_color(dcfg.drone_color)
            safety_color = _normalize_color(dcfg.safety_color or dcfg.drone_color)
            trace_color = _normalize_color(dcfg.trace_color or dcfg.drone_color)

            drones.append(
                Drone(
                    drone_id=dcfg.drone_id,
                    radius=dcfg.radius,
                    safety_zone=dcfg.safety_zone,
                    color=drone_color,
        ...
        if cfg.room is not None:
            ...
            pts: list[np.ndarray] = []
            for dcfg in cfg.drones:
                pts.append(np.asarray(dcfg.start, dtype=float))
                pts.extend([np.asarray(w, dtype=float) for w in dcfg.waypoints])
                pts.append(np.asarray(dcfg.target, dtype=float))
            for c, _r in obstacles:
                pts.append(c)

            if pts:
                stacked = np.stack(pts, axis=0)
                pmin = stacked.min(axis=0)
                pmax = stacked.max(axis=0)
            else:
                pmin = np.zeros(3)
                pmax = np.ones(3)

            margin = 1.0
            room_min = pmin - margin
            room_max = pmax + margin
    ...
    def _compute_collisions(self) -> list[dict]:
        ...
        A collision is reported when another drone enters the owner's safety sphere
        (radius = owner.safety_zone + intruder.radius) or when an obstacle center
        enters radius = owner.safety_zone + obstacle.radius.
        """
        events: list[dict] = []
            ...
            for j, intr in enumerate(self.drones):
                ...
                if dist <= threshold:
                    events.append(
                        {
                            "kind": "drone_drone",
                            "owner": owner.drone_id,
                            "intruder": intr.drone_id,
                            "distance": dist,
                            "threshold": threshold,
                        }
                    )
    ...
    def step(self) -> None:
        ...
        try:
            ...
            # This build expects a centralized MPC coordinator. All Paper/"basic_paper" configs
            # provide `coordinator: {"type": "mpc_central", ...}`. If a scenario omits the
            # coordinator, we fail fast instead of silently running a different control scheme.
            if self.coordinator is None:
                raise RuntimeError(
                    "Simulator.step requires a coordinator (centralized MPC). "
                    "Provide `coordinator` in ScenarioConfig."
                )
            ...
            for i, d in enumerate(self.drones):
                neighbors = [
                    (
                        positions[j],
                        velocities[j],
                        self.drones[j].radius,
                        self.drones[j].safety_zone,
                        prefs[j],
                    )
                ...
                if hasattr(d.controller, "control"):
                    u = d.controller.control(
                        d.x,
                        prefs[i],
                        neighbors,
                        self.obstacles,
                        self_radius=d.radius,
            ...
            all_drone_state = {
                d.drone_id: (positions[i], velocities[i], float(d.radius))
                for i, d in enumerate(self.drones)
            }

            u_by_id = self.coordinator.solve_controls(
                drone_ids=[d.drone_id for d in self.drones],
                xs=[d.x for d in self.drones],
                prefs=prefs,
                radii=[d.radius for d in self.drones],
                safety_zones=[d.safety_zone for d in self.drones],
                controllers=[d.controller for d in self.drones],
                obstacles=self.obstacles,
                all_drone_state=all_drone_state,
            )
            ...
            for d, u in zip(self.drones, us, strict=True):
                ...
                # We clamp the position and zero the velocity component(s) that hit a wall.
                p = d.position()
                pmin = self.room_min + d.radius
                pmax = self.room_max - d.radius

                p_clamped = np.clip(p, pmin, pmax)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "dt": self.dt,
            "room": {
                "min": self.room_min.tolist(),
                "max": self.room_max.tolist(),
            },
            "drones": [
                {
                    "drone_id": d.drone_id,
