...
class CentralMPCAgent(Controller):
    ...
    This controller is not meant to be used standalone; it exposes `central_cost(...)`
    which a coordinator can sum over drones.

    Note: `q_vel` is intentionally non-zero by default to discourage the optimizer from
    accelerating to very high speeds when the horizon is short.
    """
