...
def render(
    ...
    png = render_png(
        room_min=_sim.room_min,
        room_max=_sim.room_max,
        drone_positions=[d.position() for d in _sim.drones],
        drone_radii=[d.radius for d in _sim.drones],
        drone_safety_zones=safety_zones,
        drone_colors=[d.color for d in _sim.drones],
        safety_colors=[d.safety_color for d in _sim.drones],
        trace_colors=[d.trace_color for d in _sim.drones],
        drone_traces=traces,
        obstacles=_sim.obstacles,
        step_count=_sim.step_count,
        compute_time_s=_sim.compute_time_s,
        width=width,
        height=height,
        dpi=dpi,
        elev=elev,
