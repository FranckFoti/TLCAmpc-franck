...
class TestVelocityConstraintsSingle:
    ...
    def test_below_vmax_satisfied(self):
        ...
        H = 3
        vc = VelocityConstraints(horizon=H)
        drone = _make_drone(v_max=5.0)
        # speed = sqrt(3) ≈ 1.73 m/s, well below 5.0
        v_pred = np.ones((H, 3))
        values = np.array([])

        result = vc.evaluate_single(drone, v_pred, values)
    ...
    def test_appends_to_existing_values(self):
        ...
        result = vc.evaluate_single(drone, v_pred, existing)
...
class TestVelocityConstraintsMulti:
    ...
    def test_multiple_drones_below_vmax(self):
        ...
        v_pred = np.ones((2, H, 3))  # speed = sqrt(3) ≈ 1.73
        values = np.array([])

        result = vc.evaluate_multi(drones, v_pred, values)
    ...
    def test_mixed_velocities(self):
        ...
        v_pred = np.ones((2, H, 3)) * 2.0  # speed = sqrt(12) ≈ 3.46
        values = np.array([])
    ...
    def test_appends_to_existing_values(self):
        ...
        v_pred = np.zeros((1, H, 3))
        existing = np.array([1.0, 2.0])

        result = vc.evaluate_multi(drones, v_pred, existing)
...
class TestMovingObstacleAvoidanceSingle:
    ...
    def test_far_neighbor_satisfied(self):
        ...
        H = 3
        mac = MovingObstacleAvoidanceConstraints(horizon=H)
        drone = _make_drone(safety_zone=0.5)
        pred_pos = np.zeros((H, 3))
        neighbor_traj = np.ones((H, 3)) * 10.0
        neighbors = {"n1": (neighbor_traj, 0.5)}
        values = np.array([])

        result = mac.evaluate_single(drone, pred_pos, neighbors, values)
    ...
    def test_close_neighbor_violated(self):
        ...
        neighbor_traj = np.tile(np.array([0.5, 0.0, 0.0]), (H, 1))
    ...
    def test_no_neighbors_empty(self):
        ...
        result = mac.evaluate_single(drone, pred_pos, {}, values)
    ...
    def test_multiple_neighbors(self):
        ...
        pred_pos = np.zeros((H, 3))
        neighbors = {
            "n1": (np.ones((H, 3)) * 10.0, 0.5),
            "n2": (np.ones((H, 3)) * 20.0, 0.5),
        }
        values = np.array([])

        result = mac.evaluate_single(drone, pred_pos, neighbors, values)

        assert result.shape == (H * 2,)
        assert np.all(result > 0)
...
class TestMovingObstacleAvoidanceMulti:
    ...
    def test_two_drones_overlapping(self):
        ...
        pred_pos = {
            "d1": np.zeros((H, 3)),
            "d2": np.tile(np.array([0.5, 0.0, 0.0]), (H, 1)),
        }
        values = np.array([])
    ...
    def test_get_neighbor_trajectories(self):
        ...
        neighbors = mac._get_neighbor_trajectories("d1", drones, pred_pos)
...
class TestObstacleAvoidanceSingle:
    ...
    def test_far_from_obstacle_satisfied(self):
        ...
        result = oac.evaluate_single(drone, pred_pos, obstacles, values)
    ...
    def test_no_obstacles_returns_zeros(self):
        """No obstacles produces a zero-length result from _evaluate, but shape=(H,)."""
        H = 3
        oac = ObstacleAvoidanceConstraints(horizon=H)
        drone = _make_drone()
        pred_pos = np.zeros((H, 3))
        ...
        result = oac.evaluate_single(drone, pred_pos, [], values)

        # _evaluate returns np.zeros(H) even with no obstacles
        assert result.shape == (H,)
    ...
    def test_appends_to_existing_values(self):
        ...
        result = oac.evaluate_single(drone, pred_pos, obstacles, existing)

        assert result.shape == (1 + H,)
        assert result[0] == 7.0
...
class TestObstacleAvoidanceMulti:
    ...
    def test_two_drones_far_from_obstacle(self):
        ...
        result = oac.evaluate_multi(drones, pred_pos, obstacles, values)
    ...
    def test_one_drone_close_to_obstacle(self):
        ...
        drones = [d1, d2]
        pred_pos = {
            "d1": np.zeros((H, 3)),  # at origin
            "d2": np.ones((H, 3)) * 100.0,  # far away
        }
...
class TestRoomConstraintsSingleBox:
    ...
    def test_inside_room_satisfied(self):
        ...
        drone = _make_drone(safety_zone=1.0)
        pred_pos = np.zeros((H, 3)) + 5.0  # center of [0,10]^3
    ...
    def test_outside_room_violated(self):
        ...
        rc = RoomConstraints(horizon=H)
        drone = _make_drone(safety_zone=1.0)
        pred_pos = np.zeros((H, 3)) - 5.0  # well outside [0,10]^3
        room_min = np.array([0.0, 0.0, 0.0])
        room_max = np.array([10.0, 10.0, 10.0])
        values = np.array([])
...
class TestRoomConstraintsSingleSphere:
    ...
    def test_inside_sphere_satisfied(self):
        ...
        result = rc.evaluate_single(drone, pred_pos, room_max=10.0, room_min=0.0, values=values, room_is_sphere=True)
    ...
    def test_outside_sphere_violated(self):
        ...
        rc = RoomConstraints(horizon=H)
        drone = _make_drone(safety_zone=1.0)
        pred_pos = np.zeros((H, 3)) + 5.0  # dist from origin = sqrt(75) ≈ 8.66
        values = np.array([])

        result = rc.evaluate_single(drone, pred_pos, room_max=2.0, room_min=0.0, values=values, room_is_sphere=True)
...
class TestRoomConstraintsMultiSphere:
    ...
    def test_two_drones_inside_sphere(self):
        ...
        result = rc.evaluate_multi(drones, pred_pos, room_max=10.0, room_min=0.0, values=values, room_is_sphere=True)
